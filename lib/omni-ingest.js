...
export async function processOmniRows(rows, { mode = "pull", source = "omni_api" } = {}) {
  ...
  // Studio Org names that signal a test/demo account — skip at ingest
  const DEMO_ORG_PATTERNS = /\b(demo|test|sample|example|qa|staging|sandbox|fake|dummy|trial|placeholder|lorem|foobar|acme)\b/i;
  const DEMO_EMAIL_PATTERNS = /^(test|demo|sample|qa|fake|dummy|noreply|no-reply|admin|info|support|hello|contact)\+?[\w.]*@/i;
  ...
  for (const row of rows) {
    ...
    // Skip obvious demo/test accounts
    if (DEMO_ORG_PATTERNS.test(studioOrg) || DEMO_ORG_PATTERNS.test(company) || DEMO_EMAIL_PATTERNS.test(email)) {
      results.skipped++;
      continue;
    }
    validRows.push({ row, email, name, company, studioOrg });
  }

  // ── 1b. Build paid-org flag map across the full batch ────────────────────
  // If any member of a Studio Org is on a paid tier, flag others from that org.
  const PAID_TIERS = new Set(["TEAM", "BUSINESS", "ENTERPRISE", "BUSINESS_PLUS"]);
  const orgTierMap = new Map(); // studioOrg → Set<tier>
  for (const { row, studioOrg } of validRows) {
    if (!studioOrg) continue;
  ...
  // Orgs where at least one member is on a paid plan
  ...
  // ── 3b. Refresh extraContext for active existing leads ────────────────────
  // Only touch leads that have a draft ready to send OR belong to a tracked
  // account. Everything else stays untouched — we are not building a CRM.
  const existingRows = validRows.filter(({ email }) => existingEmailSet.has(email));
  if (existingRows.length > 0) {
    const rowByEmail = new Map(
      existingRows.map(({ row, email, studioOrg }) => [email, { row, studioOrg }])
    );
    const existingEmails = existingRows.map(r => r.email);

    // Fetch only the leads worth refreshing — has draft or tracked account
    const activeLeads = [];
    for (let i = 0; i < existingEmails.length; i += 900) {
      const chunk = existingEmails.slice(i, i + 900);
      const rows = await db.query.leads.findMany({
        where: inArray(leads.email, chunk),
        with: { account: true },
      });
      activeLeads.push(...rows.filter(l =>
        l.draftStatus === "done" || l.account?.tracked === "1"
      ));
    }

    for (const lead of activeLeads) {
      const match = rowByEmail.get(lead.email);
      if (!match) continue;
      const { row, studioOrg } = match;
      const orgHasPaidMembers = studioOrg ? paidOrgs.has(studioOrg) : false;
      const freshContext = buildExtraContext(row, studioOrg, orgHasPaidMembers);
      if (!freshContext) continue;
      try {
        await db.update(leads)
          .set({ extraContext: freshContext, updatedAt: now })
          .where(eq(leads.id, lead.id));
        results.refreshed = (results.refreshed ?? 0) + 1;
      } catch (err) {
        results.errors.push({ email: lead.email, error: `refresh: ${err.message}` });
      }
    }

    console.log(`[omni-ingest] refreshed extraContext for ${results.refreshed ?? 0} active existing leads`);
  }
  ...
  const leadValues = [];
  for (const { row, email, name, company, studioOrg } of newRows) {
    ...
    const orgHasPaidMembers = studioOrg && paidOrgs.has(studioOrg);
    const extraContext = buildExtraContext(row, studioOrg, orgHasPaidMembers);
...
// Google Sheets exports booleans as "TRUE"/"FALSE" strings; Omni API may send
// actual booleans or "true"/"false". Normalise all of them in one place.
...
function buildExtraContext(row, studioOrg = "", orgHasPaidMembers = false) {
  const parts = [];
  if (studioOrg) parts.push(`Studio Org: ${studioOrg}`);
  if (orgHasPaidMembers) parts.push("⚠️ Org has paid members — verify if net-new");
  if (row["Subscription Tier"]) parts.push(`Tier: ${row["Subscription Tier"]}`);
  if (isTruthy(row["Is Using Router"])) parts.push("Router: yes");
  ...
  if (isTruthy(row["Has Router Operations Last 7days"])) parts.push("Router active last 7d");
  if (isTruthy(row["Is Using Schema Checks"])) parts.push("Schema Checks: yes");
  if (isTruthy(row["Is Using Connectors"])) parts.push("Connectors: yes");
